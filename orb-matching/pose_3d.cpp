...
int main(int argc, char **argv){
    ...
    assert(img_left.data != nullptr && img_right.data != nullptr);
     ...
     find_feature_matches(img_left, img_right, keypoints_1, keypoints_2, matches);
     cout << "find " << matches.size() << "in mathes " << endl;

     // 建立3D点
     Mat d1 = imread(argv[3], CV_LOAD_IMAGE_COLOR);
     Mat K = (Mat_<double>(3, 3) << 520.9, 0, 325.1, 0, 521.0, 249.7, 0, 0, 1);
     vector<Point3f> pts_3d;
     vector<Point2f> pts_2d;
     for (DMatch m:matches){
         ushort d = d1.ptr<unsigned short>(int(keypoints_1[m.queryIdx].pt.y))[int(keypoints_1[m.queryIdx].pt.x)];       // 获得对应的深度信息
         if (d == 0){
             continue;
         }
         float dd = d / 5000.0;
         Point2d p1 = pixel2cam(keypoints_1[m.queryIdx].pt, K);     // 转换到归一化像素平面
     ...
     Mat r, t;
     solvePnP(pts_3d, pts_2d, K, Mat(), r, t, false);           // 调用OpenCV的PnP来进行求解

     Mat R;
     cv::Rodrigues(r, R);           // 利用罗德里格斯公式将旋转向量转换为R旋转矩阵
     cout << "R =" << endl << R << endl;
     cout << "t= " << endl << t << endl;
